from wtforms import Form, RadioField, StringField, PasswordField, SelectField, validators
...
class RegistrationForm(Form):
    ...
    school_id = SelectField('School', choices=[('1','Columbia'),
        ('2','NYU'), ('3','Fordham'), ('4','Hunter'), ('5','Brooklyn'),
        ('6','Baruch'), ('7','Pace'), (8,'Queens'), (9,'Juilliard'),
        (10,'Parsons')])
    year = RadioField('Year', choices=[('UG1', 'Freshman'),
        ('UG2', 'Sophomore'), ('UG3', 'Junior'), ('UG4','Senior'),
        ('G', 'Graduate'), ('PROF', 'Professor')])
    ...
    def validate(self):
      ...
      if not super(RegistrationForm, self).validate():
          return False
      res = conn.execute("SELECT U.email FROM users_belong_to U "
                         "WHERE U.email = '{}'".format(self.email.data))
      if res.rowcount == 1:
          self.email.errors.append('email already exists')          
          conn.close()
          return False

      res1 = conn.execute("SELECT U.uid FROM users_belong_to U "
                         "WHERE U.uid = '{}'".format(self.user_id.data))
      if res1.rowcount == 1:
